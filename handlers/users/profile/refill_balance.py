...
from utils.db_api.quick_commands.user_inquiries import update_balance
...
async def back_profile(call: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state("other_amount#" + call.data.split('#')[1])


@dp.message_handler(lambda message: not message.text.isdigit(), state="other_amount")
...
@dp.message_handler(lambda message: message.text.isdigit(), state="other_amount")
